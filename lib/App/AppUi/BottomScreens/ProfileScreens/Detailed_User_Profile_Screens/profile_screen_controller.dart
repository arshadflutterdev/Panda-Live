...
class ProfileController extends GetxController {
  ...
  //private videos
  var privateVideos = <VideoModel>[].obs;
  ...
  void bindAllVideoStreams() {
    // 1. Sirf Public Videos fetch karne ke liye (isPrivate: false)
    ...
    // 2. Sirf Private Videos fetch karne ke liye (isPrivate: true)
    privateVideos.bindStream(
      _db
          .collection('videos')
          .where('uid', isEqualTo: targetUid)
          .where('isPrivate', isEqualTo: true) // <--- Sirf private videos
          .snapshots()
          .map(
            (query) =>
                query.docs.map((doc) => VideoModel.fromSnap(doc)).toList(),
