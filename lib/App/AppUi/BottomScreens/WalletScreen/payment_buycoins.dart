...
class _PaymentFormScreenState extends State<PaymentFormScreen> {
  ...
  Future<void> _submitRequest(bool isArabic) async {
    if (_trxIdController.text.trim().isEmpty) {
      Get.snackbar(
        isArabic ? "خطأ في التحقق" : "Validation Error",
        isArabic
            ? "يرجى إضافة معرّف المعاملة (TrxID)."
    ...
    try {
      ...
      // Add document to 'coin_purchase_requests' without screenshotUrl
      ...
      Get.defaultDialog(
        ...
        middleText: isArabic
            ? "تم تقديم طلبك بنجاح. سيتم التحقق منه قريباً."
            : "Request submitted successfully. Admin will verify the TrxID soon.",
        onConfirm: () {
  ...
  // Helper widgets remain largely the same, but instructions are slightly updated
  Widget _buildInstructionCard(bool isArabic, String adminShortId) {
    return Container(
      width: double.infinity,
      padding: const EdgeInsets.all(20),
          ...
          RichText(
            text: TextSpan(
              ...
              children: [
                TextSpan(
                  text: isArabic
                      ? "يرجى تحويل ($planAmount \$) إلى معرف Binance: "
                      : "Send ($planAmount \$) to Binance ID: ",
                ),
                TextSpan(
                  text: adminShortId,
                  style: const TextStyle(
                ...
                TextSpan(
                  text: isArabic
                      ? ". الصق معرّف العملية (TrxID) أدناه للتأكيد."
                      : ". Paste your TrxID below to confirm.",
                ),
              ],
            ),
          ),
